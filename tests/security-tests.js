...
async function runSecurityTests() {
  ...
  await test('JWT Secret is secure', () => {
    ...
    if (jwtSecret.includes('development') && process.env.NODE_ENV === 'production') {
  ...
  await test('Database connection secure', () => {
    ...
    if (!dbUrl.includes('postgresql://')) throw new Error('Invalid database URL format');
  ...
  await test('AI moderation performance check', async () => {
    ...
    if (duration > 500) {
      throw new Error(`AI moderation too slow: ${duration}ms (should be <500ms)`);
    }
  ...
  if (successRate >= 95) {
    ...
    console.log('Most security measures are in place but some improvements needed.');
  } else {
